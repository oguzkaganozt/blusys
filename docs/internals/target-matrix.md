...
| Category | Modules | Notes |
|----------|---------|-------|
| **Product API** | `blusys::app` (`app_spec`, `app_ctx`, `app_fx`, `entry`), profiles (`host`, `headless`, `st7735`), capabilities (`connectivity`, `storage`, `bluetooth`, `ota`, `diagnostics`, `telemetry`, `lan_control`, `usb`) | All three targets |
