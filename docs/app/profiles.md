...
The **blusys_framework** component defines shared **menuconfig** choices for SPI display profiles (dashboard-class ILI9341, ILI9488, or ST7735, plus optional QEMU RGB; interactive-controller ST7735 vs ST7789). Individual examples add only product-specific options (e.g. WiFi strings on gateway).
...
Host SDL builds for dashboard-class examples use `BLUSYS_DASHBOARD_HOST_DISPLAY_PROFILE` in `host/CMakeLists.txt` (0 = 320×240 logical, 1 = 480×320, 2 = 160×128 ST7735-class). The interactive controller quickstart uses `BLUSYS_IC_HOST_DISPLAY_PROFILE` for its ST7735/ST7789 host window matrix.
