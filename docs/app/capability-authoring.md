...
Every capability derives from `capability_base` in `blusys/app/capability.hpp`:
...
| `0x0800–0x08FF` | mqtt_host (SDL) |
| `0x0900–0x09FF` | product custom |

Add a new kind to `capability_kind` and reserve a new range in the header comment when you add a capability to the framework. Product-custom capabilities live in the `0x0900` block and flow through `app_spec::map_event` as `integration_passthrough` until mapped.

## Shape to follow

Model new capabilities on `storage_capability` (`blusys/app/capabilities/storage.hpp` + `src/app/capabilities/storage.cpp`) — the smallest reference. Conventions:

1. **One header, two implementations.** Public header declares shared `<capability>_event` enum, a `<capability>_status` struct, and a `<capability>_config`. The class body gates device vs host with `#ifdef ESP_PLATFORM`; device code lives in `src/app/capabilities/<name>.cpp`, host stub in `<name>_host.cpp`.
