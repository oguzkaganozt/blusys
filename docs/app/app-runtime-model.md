...
Capabilities are **started** before `on_init` and **polled** from the runtime step; integration events they post use the reserved `event_id` ranges documented in `blusys/framework/capabilities/` and the typed helpers in `blusys/framework/app/` (or `integration_dispatch.hpp` alone).
