...
All root scripts source `idf-common.sh` and are the safest default entrypoints:

- configure one example: `./configure.sh examples/<name> [esp32|esp32c3|esp32s3]`
- build one example: `./build.sh examples/<name> [esp32|esp32c3|esp32s3]`
- flash one example: `./flash.sh examples/<name> [port] [esp32|esp32c3|esp32s3]`
- monitor one example: `./monitor.sh examples/<name> [port] [esp32|esp32c3|esp32s3]`
- full build/flash/monitor: `./run.sh examples/<name> [port] [esp32|esp32c3|esp32s3]`
- remove one target build dir: `./clean.sh examples/<name> [esp32|esp32c3|esp32s3]`
- build every example for every target: `./build_examples.sh`
...
- `run.sh` and `flash.sh` auto-detect a serial port only when exactly one `/dev/ttyUSB*` or `/dev/ttyACM*` exists; otherwise pass the port explicitly

## Environment Gotchas

- ESP-IDF export is required before direct `idf.py` use; helper scripts do it for you
- `idf-common.sh` assumes:
...
- full release-style verification is `./build_examples.sh`
