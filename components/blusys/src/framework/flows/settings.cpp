                                  ...
                                  lv_group_t **group_out)
{
    ...
    auto page = page_create({.scrollable = true});
    ...
    for (std::size_t i = 0; i < cfg->item_count; ++i) {
        ...
        case setting_type::info: {
            blusys::key_value_create(page.content, {
                .key = item.label,
        ...
        case setting_type::toggle: {
            ...
            blusys::toggle_config tcfg{};
            tcfg.initial   = item.toggle_initial;
            tcfg.on_change = on_toggle_changed;
            tcfg.user_data = ic;
            lv_obj_t *tgl = blusys::toggle_create(row, tcfg);
            if (tgl != nullptr) {
        ...
        case setting_type::dropdown: {
            ...
            dcfg.user_data = ic;
            lv_obj_t *dd = blusys::dropdown_create(page.content, dcfg);
            if (dd != nullptr) {
        ...
        case setting_type::action_button: {
            item_cb_ctx *ic = acquire_item_ctx(cfg, i);
            blusys::button_config bcfg{};
            bcfg.label     = item.button_label != nullptr ? item.button_label : item.label;
            bcfg.variant   = blusys::button_variant::secondary;
            bcfg.on_press  = on_button_pressed;
            bcfg.user_data = ic;
            lv_obj_t *btn = blusys::button_create(page.content, bcfg);
            if (btn != nullptr) {
