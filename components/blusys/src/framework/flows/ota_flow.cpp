...
#include "blusys/framework/flows/ota_flow.hpp"
                                  ...
                                  lv_group_t **group_out)
{
    ...
    // Progress bar.
    blusys::progress_config pcfg{};
    pcfg.min      = 0;
    pcfg.max      = 100;
    pcfg.initial  = 0;
    pcfg.show_pct = true;
    lv_obj_t *progress = blusys::progress_create(page.content, pcfg);
                        ...
                        const ota_flow_config &config)
{
    if (handles.progress_bar != nullptr) {
        blusys::progress_set_value(handles.progress_bar,
