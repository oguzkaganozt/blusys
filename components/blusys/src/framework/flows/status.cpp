...
#include "blusys/framework/flows/status.hpp"
...
lv_obj_t *alert_create(lv_obj_t *parent, const alert_config &config)
{
    ...
    lv_color_t bg;
    switch (config.level) {
    case blusys::badge_level::success: bg = t.color_success; break;
    case blusys::badge_level::warning: bg = t.color_warning; break;
    case blusys::badge_level::error:   bg = t.color_error;   break;
...
lv_obj_t *confirmation_create(lv_obj_t *parent,
                                ...
                                blusys::press_cb_t on_confirm,
                                void *confirm_ctx,
                                blusys::press_cb_t on_cancel,
                                void *cancel_ctx)
{
    const auto &t = blusys::theme();
    ...
    if (config.cancel_label != nullptr) {
        ...
        btn_cfg.variant   = blusys::button_variant::ghost;
        btn_cfg.on_press  = on_cancel;
        btn_cfg.user_data = cancel_ctx;
        blusys::button_create(btn_row, btn_cfg);
    }

    if (config.confirm_label != nullptr) {
        blusys::button_config btn_cfg{};
        btn_cfg.label     = config.confirm_label;
        btn_cfg.variant   = blusys::button_variant::primary;
