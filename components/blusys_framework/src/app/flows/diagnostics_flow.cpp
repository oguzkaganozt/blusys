                                    ...
                                    const diagnostics_display_config &config)
{
    ...
    if (config.show_heap) {
        lv_obj_t *heap = blusys::ui::key_value_create(panel, {.key = "Free Heap", .value = "\xe2\x80\x94"});
        lv_obj_t *min_heap = blusys::ui::key_value_create(panel, {.key = "Min Heap", .value = "\xe2\x80\x94"});
        if (handles != nullptr) {
    ...
    if (config.show_uptime) {
        lv_obj_t *uptime = blusys::ui::key_value_create(panel, {.key = "Uptime", .value = "\xe2\x80\x94"});
        if (handles != nullptr) { handles->uptime_kv = uptime; }
    }

    if (config.show_chip) {
        lv_obj_t *chip = blusys::ui::key_value_create(panel, {.key = "Chip", .value = "\xe2\x80\x94"});
        lv_obj_t *idf = blusys::ui::key_value_create(panel, {.key = "IDF", .value = "\xe2\x80\x94"});
        if (handles != nullptr) {
    ...
    if (config.show_flash) {
        lv_obj_t *flash = blusys::ui::key_value_create(panel, {.key = "Flash", .value = "\xe2\x80\x94"});
                               ...
                               const diagnostics_snapshot &snapshot)
{
    ...
    if (handles.heap_kv != nullptr) {
        ...
        blusys::ui::key_value_set_value(handles.heap_kv, buf);
    }

    if (handles.min_heap_kv != nullptr) {
        std::snprintf(buf, sizeof(buf), "%lu B",
                      static_cast<unsigned long>(snapshot.min_free_heap));
        blusys::ui::key_value_set_value(handles.min_heap_kv, buf);
    }
    ...
    if (handles.uptime_kv != nullptr) {
        ...
        blusys::ui::key_value_set_value(handles.uptime_kv, buf);
    }
    ...
    if (handles.chip_kv != nullptr) {
        ...
        blusys::ui::key_value_set_value(handles.chip_kv, buf);
    }

    if (handles.idf_kv != nullptr) {
        blusys::ui::key_value_set_value(handles.idf_kv, snapshot.idf_version);
    }
    ...
    if (handles.flash_kv != nullptr) {
        ...
        blusys::ui::key_value_set_value(handles.flash_kv, buf);
    }
}
