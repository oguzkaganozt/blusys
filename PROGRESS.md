...
- current phase: `Phase 8 closed — blusys_all.h removed, example ecosystem migrated, framework_encoder_basic added; next: Phase 9 validation`
...
1. **Phase 9 — Validation and migration notes.** Run the validation chain (PC + LVGL + SDL2 → ESP-IDF QEMU → real hardware) for the sample interactive product, the headless product, and the framework examples. Capture first-boot success on real hardware. Measure input → feedback latency for the interactive product. Write the V5→V6 migration notes for downstream consumers.
2. **Bridge `blusys_framework` into the host harness.** Extend `scripts/host/CMakeLists.txt` to compile the framework C++ sources alongside `hello_lvgl` so the widget kit can be exercised on host. This unblocks visual snapshot testing and faster widget iteration. Also enables `framework_encoder_basic` to run with SDL2 keyboard input mapped as encoder rotation, which is currently the only framework example explicitly designed to support host harness encoder simulation.
3. **Tag `v6.0.0` once Phase 9 lands** so scaffolded products can pull the platform from the canonical git URL without `override_path`. Until then, scaffolded products must point at the local checkout via `override_path` for verification.
4. Keep `platform-transition/` and repo guidance docs aligned as Phase 9 work lands.
