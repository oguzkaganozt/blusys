...
/*
 * Concurrency test: two worker tasks race against the timer ISR.
 *
 * Worker A: rapidly calls blusys_timer_stop() / blusys_timer_start() to
 *   exercise the start/stop path while the ISR may be firing.
 *
 * Worker B: rapidly calls blusys_timer_set_callback() to swap between two
 *   callbacks while the ISR may be firing.
 *
 * Both workers complete their iterations and notify the main task.
 * After both finish, the test verifies the timer can still be stopped
 * and closed cleanly.
 *
 * Pass criteria: no invalid error codes from any API call, clean final stop,
 * at least one callback tick received during the test.
 */
...
static void blusys_concurrency_timer_worker_b(void *arg)
{
    ...
    /* Two static contexts, one per callback variant, shared across iterations */
