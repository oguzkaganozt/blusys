...
def validate_modules(modules, errors):
    ...
    for i, mod in enumerate(modules):
        ...
        if tier not in VALID_TIERS:
            errors.append(f"{prefix} ({name}): invalid tier '{tier}', expected one of {VALID_TIERS}")

        # Support
        support = mod.get("support")
        if support not in VALID_SUPPORT:
            errors.append(f"{prefix} ({name}): invalid support '{support}', expected one of {VALID_SUPPORT}")
...
def validate_examples(examples, errors):
    ...
    for i, ex in enumerate(examples):
        ...
        if category not in VALID_CATEGORIES:
            errors.append(f"{prefix} ({name}): invalid category '{category}', expected one of {VALID_CATEGORIES}")

        # Visibility
        visibility = ex.get("visibility")
        if visibility not in VALID_VISIBILITY:
            errors.append(f"{prefix} ({name}): invalid visibility '{visibility}', expected one of {VALID_VISIBILITY}")
        ...
        if category == "quickstart" and visibility != "public":
            errors.append(f"{prefix} ({name}): quickstart examples must have visibility=public")
        if category == "quickstart" and not ex.get("ci_pr"):
            errors.append(f"{prefix} ({name}): quickstart examples must have ci_pr=true")
        if category == "validation" and visibility != "internal":
            errors.append(f"{prefix} ({name}): validation examples must have visibility=internal")

    # Check for orphaned directories (exist on disk but not in inventory)
    orphans = actual_dirs - inventory_names
    for orphan in sorted(orphans):
        errors.append(f"orphan example directory '{orphan}' exists on disk but has no inventory entry")
...
def validate_docs(docs, errors):
    ...
    for i, doc in enumerate(docs):
        ...
        if section not in VALID_SECTIONS:
            errors.append(f"{prefix} ({path}): invalid section '{section}', expected one of {VALID_SECTIONS}")

        # Visibility
        visibility = doc.get("visibility")
        if visibility not in VALID_VISIBILITY:
            errors.append(f"{prefix} ({path}): invalid visibility '{visibility}', expected one of {VALID_VISIBILITY}")
