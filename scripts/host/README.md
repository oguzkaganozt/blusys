...
| `interactive_archetype_reducer_smoke` | Exercises `blusys_examples::clamp_percent` shared with interactive archetype reducers |
| `panel_connectivity_map_smoke` | `panel_connectivity_event_triggers_sync` parity with the interactive_panel `map_event` bridge |
...
- **`undefined reference to blusys::ui::...` from `bindings.cpp`** — add the matching `blusys::ui` widget `.cpp` to `cmake/blusys_framework_ui_sources.cmake` (`BLUSYS_FRAMEWORK_UI_SRC_SHARED` and matching `BLUSYS_FRAMEWORK_HOST_WIDGETKIT_REL` entries). Interactive host examples include `cmake/blusys_framework_host_widgetkit.cmake`, which reads that file. Run `python3 scripts/check-framework-ui-sources.py` after edits.
